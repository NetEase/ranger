 ...
 *
 * JAAS.inmemory.KafkaClient.loginModuleName = com.sun.security.auth.module.Krb5LoginModule
 * JAAS.inmemory.KafkaClient.loginModuleControlFlag = required
 * JAAS.inmemory.KafkaClient.option.useKeyTab = true
 * JAAS.inmemory.KafkaClient.option.storeKey = true
 * JAAS.inmemory.KafkaClient.option.serviceName = kafka
 * JAAS.inmemory.KafkaClient.option.keyTab = /etc/security/keytabs/kafka_client.keytab
 * JAAS.inmemory.KafkaClient.option.principal = kafka-client-1@EXAMPLE.COM

 * JAAS.inmemory.MyClient.0.loginModuleName = com.sun.security.auth.module.Krb5LoginModule
 * JAAS.inmemory.MyClient.0.loginModuleControlFlag = required
 * JAAS.inmemory.MyClient.0.option.useKeyTab = true
 * JAAS.inmemory.MyClient.0.option.storeKey = true
 * JAAS.inmemory.MyClient.0.option.serviceName = kafka
 * JAAS.inmemory.MyClient.0.option.keyTab = /etc/security/keytabs/kafka_client.keytab
 * JAAS.inmemory.MyClient.0.option.principal = kafka-client-1@EXAMPLE.COM
 * JAAS.inmemory.MyClient.1.loginModuleName = com.sun.security.auth.module.Krb5LoginModule
 * JAAS.inmemory.MyClient.1.loginModuleControlFlag = optional
 * JAAS.inmemory.MyClient.1.option.useKeyTab = true
 * JAAS.inmemory.MyClient.1.option.storeKey = true
 * JAAS.inmemory.MyClient.1.option.serviceName = kafka
 * JAAS.inmemory.MyClient.1.option.keyTab = /etc/security/keytabs/kafka_client.keytab
 * JAAS.inmemory.MyClient.1.option.principal = kafka-client-1@EXAMPLE.COM
 ...
 *  };
 *
 *  Here is the syntax for kafka.properties to add JAAS configuration:
 *
 *  The property name has to begin with   'JAAS.inmemory.' +  clientId   (in case of Kafka client, it expects the clientId to be  KafkaClient)
 *  The following property must be there to specify the JAAS loginModule name,  'JAAS.inmemory.' +  clientId  + '.loginModuleName'
 *  The following optional property should be set to specify the loginModuleControlFlag,  'JAAS.inmemory.' + clientId + '.loginModuleControlFlag'
 *  		Default value :  required ,  Possible values:  required, optional, sufficient, requisite
 *  Then you can add additional optional parameters as options for the configuration using the following syntax: 'JAAS.inmemory.' + clientId + '.option.' + <optionName>  = <optionValue>
 *
 *  The current setup will lookup JAAS configration from the kafka.properties first, if not available, it will delegate to the original configuration 
 *  (what was defined before the InMemoryJAASConfiguration.init() was  called)
 *
 */

public final class InMemoryJAASConfiguration extends Configuration {

    private static final Logger LOG = Logger.getLogger(InMemoryJAASConfiguration.class);

    private static final String JAAS_CONFIG_PREFIX_PARAM = "JAAS.inmemory.";
